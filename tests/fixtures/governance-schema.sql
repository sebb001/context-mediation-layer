-- governance store schema v2
-- 15 tables: scopes, domains, actors, roles, actor_role_bindings, actor_sessions,
--            intents, interpretations, actions, expertise_signals, watermarks,
--            events, claims, reports, contracts
...
-- Role contracts (trust envelopes that can be bound to specific actors/surfaces)
...
-- Contracts (canonical contract matter; no vault/file projections)
...
-- Actor-role bindings (which provisioned actor may assume a role through which surface/credential)
...
-- Actor sessions (ephemeral liveness; accountable records reference actors, not sessions)
...
-- Interpretations (what a domain understood an intent to mean)
...
-- Expertise signals (who has weighed in)
...
-- Watermarks (per-actor read position for compute economy)
...
-- Events (append-only immutable decision trail)
...
-- Claims (advisory work-in-progress signals)
...
-- Reports (reference artifacts: compressions, tech notes, snapshots — NOT interpretations)
